...
import {
  NOTIFICATIONS, STUDENT_PROFILE, ACCOUNTS, type Account,
  BOT_GREET_TEXT, CHAT_SUGGESTIONS, mockReply,
  type Notification,
} from "../data/mockData";
import { AdminApp } from "./AdminSections";
import { HelpButton, SidebarLogo, getInitials, abbreviateName } from "./shared";
import {
...
function AccountPickerModal({ onLogin }: { onLogin: (acc: Account) => void }) {
  return (
...
// ─── Login Page ───────────────────────────────────────────────────────────────
function LoginPage({ onLogin }: { onLogin: (acc: Account) => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm mx-4">
        ...
        <div className="rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-8 pt-8 pb-7 text-center" style={{ background: "linear-gradient(135deg,#11284D 0%,#264B6F 100%)" }}>
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-3" style={{ background: "rgba(255,255,255)" }}>
              <img src={logoImg} alt="CampUS" style={{ mixBlendMode: "multiply", width: 60, height: 60 }} className="object-contain" />
            </div>
      ...
      {showPicker && <AccountPickerModal onLogin={acc => { setShowPicker(false); onLogin(acc); }} />}
    </div>
  );
}
...
export default function App() {
  ...
  const [loggedAccount, setLoggedAccount] = useState<Account>(ACCOUNTS[1]);
  ...
  if (userRole === "admin") return <AdminApp onLogout={() => setIsLoggedIn(false)} HelpButton={HelpButton} adminProfile={loggedAccount} />;
    ...
    <div className="flex h-screen overflow-hidden bg-background" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
        ...
        <div className={`p-4 flex items-center gap-3 flex-shrink-0 ${sidebarOpen ? "" : "justify-center"}`}>
          <div className="w-9 h-9 rounded-full flex-shrink-0 overflow-hidden flex items-center justify-center" style={{ background: "rgba(213,179,112,0.12)", border: "2px solid rgba(213,179,112,0.3)" }}>
            {studentAvatarUrl
              ...
              : <span className="text-sm font-bold" style={{ color: "var(--accent)", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{getInitials(studentProfile.fullName)}</span>}
          </div>
          {sidebarOpen && (
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-white truncate" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{abbreviateName(studentProfile.fullName)}</div>
              <div className="text-xs text-white/40 truncate font-mono">{studentProfile.mssv}</div>
            </div>
          )}
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="ml-auto flex items-center gap-1.5">
                ...
                <div className="fixed left-3 right-3 top-[58px] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-96 bg-card border border-border rounded-xl shadow-2xl overflow-hidden" style={{ zIndex: 200 }}>
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-border">
                    {NOTIFICATIONS.map(n => {
                      ...
                      return (
                        <div key={n.id} onClick={() => { markRead(n.id); setSelectedNotif(n); setActiveSection("notifications"); setNotifOpen(false); }}
            ...
            <div className="relative" ref={avatarRef}>
              <button onClick={() => { setAvatarOpen(o => !o); setNotifOpen(false); }} className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center hover:opacity-80 transition-opacity" style={{ background: "rgba(213,179,112,0.12)", color: "var(--accent)", border: "2px solid rgba(213,179,112,0.25)", fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: "12px" }}>
                {studentAvatarUrl ? <img src={studentAvatarUrl} alt="avatar" className="w-full h-full object-cover" /> : getInitials(studentProfile.fullName)}
              </button>
              {avatarOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-card border border-border rounded-xl shadow-2xl overflow-hidden" style={{ zIndex: 50 }}>
                  <div className="px-4 py-4 flex items-center gap-3 border-b border-border">
                    ...
                    <div className="min-w-0">
                      <div className="font-semibold text-sm truncate" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{studentProfile.fullName}</div>
                      <div className="text-xs text-muted-foreground truncate">{studentProfile.officialEmail}</div>
                    </div>
                  </div>
                  <div className="py-1">
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-5 md:p-6 pb-20 md:pb-6 bg-background">
          {activeSection === "profile"        && <ProfileSection avatarUrl={studentAvatarUrl} onAvatarChange={setStudentAvatarUrl} onProfileSave={setStudentProfile} />}
